...
func New(ctx context.Context, logger slog.Logger, db database.Store, interval time.Duration) *Rolluper {
...
func (r *Rolluper) start(ctx context.Context, interval time.Duration) {
	...
	do := func() {
		...
		eg.Go(func() error {
			return r.db.InTx(func(database.Store) error {
				// Acquire a lock to ensure that only one instance of
				// the rollup is running at a time.
				ok, err := r.db.TryAcquireLock(ctx, database.LockIDDBRollup)
				if err != nil {
					return err
				}
				if !ok {
				...
				return r.db.UpsertTemplateUsageStats(ctx)
			}, nil)
		})
