...
func TestRollup_Close(t *testing.T) {
	t.Parallel()
	rolluper := dbrollup.New(context.Background(), slogtest.Make(t, nil), dbmem.New(), dbrollup.DefaultInterval)
	err := rolluper.Close()
...
func TestRollupTemplateUsageStats(t *testing.T) {
	...
	rolluper := dbrollup.New(ctx, logger, db, dbrollup.DefaultInterval)
