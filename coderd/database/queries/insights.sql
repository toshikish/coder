...
SELECT
	COALESCE((SELECT template_ids FROM templates), ARRAY[]::uuid[])::uuid[] AS template_ids, -- Includes app usage.
	COALESCE((SELECT ssh_template_ids FROM templates), ARRAY[]::uuid[])::uuid[] AS ssh_template_ids,
	COALESCE((SELECT sftp_template_ids FROM templates), ARRAY[]::uuid[])::uuid[] AS sftp_template_ids,
	COALESCE((SELECT reconnecting_pty_template_ids FROM templates), ARRAY[]::uuid[])::uuid[] AS reconnecting_pty_template_ids,
	COALESCE((SELECT vscode_template_ids FROM templates), ARRAY[]::uuid[])::uuid[] AS vscode_template_ids,
	COALESCE((SELECT jetbrains_template_ids FROM templates), ARRAY[]::uuid[])::uuid[] AS jetbrains_template_ids,
